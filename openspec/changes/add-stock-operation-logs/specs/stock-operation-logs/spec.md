...
### Requirement: 同花顺委托截图导入

系统 SHALL 允许上传同花顺当日委托截图，通过 OCR/结构化解析生成 symbol、action（买入/卖出/撤单视同 adjust）、quantity、price、fee、account、executed_at 的当日操作记录，提供可编辑预览并由用户确认后批量创建操作日志。

#### Scenario: 成功解析并导入

- **WHEN** 用户上传包含当日委托信息的同花顺截图并通过大小/格式校验
- **THEN** 系统执行 OCR 与字段解析，生成结构化记录展示在可编辑预览中，用户确认后批量写入操作日志并出现在列表顶部（受当前筛选约束）。

#### Scenario: 解析失败或日期不符

- **WHEN** 上传的截图日期非当日、无法识别必要字段，或文件不符合要求
- **THEN** 系统提示错误或需补全的字段，阻止批量创建，并提供重新上传或手工录入的入口。
